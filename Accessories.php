...
<?php
/**
 * SARAL IT SOLUTION - ACCESSORIES PAGE
 * MATCHES THE MANAGED DESIGN (ROUNDED 20PX, PURPLE THEME)
 */
require_once 'config/db.php';

try {
    // 1. GET ACTIVE FILTERS FROM URL
    ...
    // [NEW] Fetch Sub-Categories and group them by Category ID
    $subs_raw = $pdo->query("SELECT * FROM accessory_sub_categories ORDER BY name ASC")->fetchAll();
    $grouped_subs = [];
    foreach($subs_raw as $s) {
        $grouped_subs[$s['category_id']][] = $s;
    }

    // 3. BUILD DYNAMIC QUERY
    // [NEW] Added join for sub_categories
    $sql = "SELECT a.*, b.name as bname, c.name as cname, s.name as sname 
            FROM accessories a 
    ...
    $params = [];
    if ($current_brand !== 'all') { $sql .= " AND a.brand_id = ?"; $params[] = $current_brand; }
    if ($current_cat   !== 'all') { $sql .= " AND a.category_id = ?"; $params[] = $current_cat; }
    // [NEW] Filter by Sub Category
    if ($current_sub   !== 'all') { $sql .= " AND a.sub_category_id = ?"; $params[] = $current_sub; }
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Accessories | Saral IT Solution</title>
    <!-- Icons -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
     ...
     <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', sans-serif; }
        ...
        .sidebar { width: flex; flex-shrink: 0; }
        .filter-card { 
        ...
        .sub-filter-container { margin-left: 10px; border-left: 2px solid #eee; margin-bottom: 5px; }
        .sub-filter-item { 
            display: block; padding: 8px 15px; color: #777; font-size: 13px; border-radius: 0 10px 10px 0;
        }
        .sub-filter-item:hover { color: var(--primary-purple); background: #fdfdfd; }
        .sub-filter-item.active { color: var(--sale-red); font-weight: 700; background: #fff5f5; }
    ...
    <div class="wrapper">
        ...
        <aside class="sidebar">
            ...
            <div class="filter-card">
                ...
                <div class="filter-list">
                    ...
                    <?php foreach($cats_list as $c): ?>
                        <!-- Main Category Link -->
                        <a href="?cat_id=<?= $c['id'] ?>&brand_id=<?= $current_brand ?>" 
                           class="filter-item <?= ($current_cat == $c['id']) ? 'active' : '' ?>">
                            <?= htmlspecialchars($c['name']) ?>
                        </a>

                        <!-- [NEW] Sub Categories: Show only if Parent is Active -->
                        <?php if($current_cat == $c['id'] && isset($grouped_subs[$c['id']])): ?>
                            <div class="sub-filter-container">
                                <!-- Option to show All in this Category -->
                                <a href="?cat_id=<?= $c['id'] ?>&brand_id=<?= $current_brand ?>&sub_id=all" 
                                   class="sub-filter-item <?= ($current_sub == 'all') ? 'active' : '' ?>">
                                    All <?= htmlspecialchars($c['name']) ?>
                                </a>

                                <?php foreach($grouped_subs[$c['id']] as $sub): ?>
                                    <a href="?cat_id=<?= $c['id'] ?>&brand_id=<?= $current_brand ?>&sub_id=<?= $sub['id'] ?>" 
                                       class="sub-filter-item <?= ($current_sub == $sub['id']) ? 'active' : '' ?>">
            ...
            <div class="filter-card">
                ...
                <div class="filter-list">
                    <a href="?brand_id=all&cat_id=<?= $current_cat ?>&sub_id=<?= $current_sub ?>" class="filter-item <?= ($current_brand == 'all') ? 'active' : '' ?>">All Brands</a>
                    <?php foreach($brands_list as $b): ?>
                        <a href="?brand_id=<?= $b['id'] ?>&cat_id=<?= $current_cat ?>&sub_id=<?= $current_sub ?>" 
            ...
            <a href="Accessories.php" class="reset-link"><i class="fa fa-sync-alt"></i> RESET ALL FILTERS</a>
        </aside>
            ...
            <div class="product-grid">
                <?php if(empty($products)): ?>
                    <div style="grid-column: 1/-1; text-align: center; padding: 100px; background: white; border-radius: 20px;">
                        <i class="fa fa-keyboard" style="font-size: 50px; color: #eee; margin-bottom: 20px;"></i>
                        <p style="color: #999;">No accessories found matching your criteria.</p>
                        <a href="Accessories.php" style="color: var(--primary-purple); font-weight: bold;">Show All Products</a>
                    </div>
                <?php else: ?>
                    ...
                    <div class="product-card">
                        <div class="img-container">
                            ...
                            ?>
                            <img src="<?= $img_url ?>" onerror="this.src='https://via.placeholder.com/300'" alt="Accessory">
                        </div>
                        
                        <div class="brand-tag">
                            <?= htmlspecialchars($p['bname'] ?? 'Brand') ?> • 
                            <?= htmlspecialchars($p['cname'] ?? 'Accessory') ?>
                            <!-- [NEW] Show Sub Category Name if available -->
                            <?php if(!empty($p['sname'])): ?>
                                • <?= htmlspecialchars($p['sname']) ?>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="price">
                          <?php $wa_msg = "I want to buy: " . $p['name']; ?>
                          <a href="https://wa.me/9767220473?text=<?= $wa_msg ?>" target="_blank" class="wa-btn"> <span style="color:#bbb; font-size:16px;">Price on Call</span></a>
                        </div>

                        <a href="accessory-details.php?id=<?= $p['id'] ?>" class="btn-view">VIEW SPECS</a>
                    </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
