...
// --- 1. FETCH DATA FROM 'return_policy' TABLE ---
$stmt = $pdo->query("SELECT * FROM return_policy WHERE id = 1");
$row = $stmt->fetch(PDO::FETCH_ASSOC);

// Set Defaults
$heroTitle = $row['hero_title'] ?? 'Return & Refund Policy';
$heroImage = $row['hero_image'] ?? '';
$lastUpdated = $row['last_updated'] ?? 'Recently';
$content = $row['content'] ?? '<p>Our return policy details are being updated. Please check back soon.</p>';

// Dynamic Style for Hero Image
$heroStyle = "background-color: #333;"; // Default Dark Grey
if (!empty($heroImage)) {
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo htmlspecialchars($heroTitle); ?> | Mudita Store</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .container {
            ...
            border-top: 5px solid #E65100; /* Orange for Returns */
        }

        /* --- TYPOGRAPHY --- */
        .content h2, .content h3 { color: #222; margin-top: 30px; }
        ...
        /* Links */
        .content a { color: #E65100; text-decoration: underline; }
...
<body>
    ...
    <div class="hero">
        <div class="hero-content">
            ...
            <div class="date"><i class="far fa-calendar-alt"></i> Last Updated: <?php echo htmlspecialchars($lastUpdated); ?></div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="content">
            <!-- Decode and Display HTML from Database -->
            <?php 
                // Using html_entity_decode to render the HTML tags correctly
